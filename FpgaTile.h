/**
 * @file FpgaTile.h
 * @brief 单个 FPGA tile：持有该 tile 的水平/垂直导线、逻辑引脚及其四邻指针。
 *
 * 职责：
 *  - 描述 tile 坐标与相邻 tile 关系；
 *  - 通过 generateContents() 创建 V_WIRE/H_WIRE 与 CB_WIRE 逻辑引脚；
 *  - 通过 populateSwitchbox() 建立开关块内的导线连接；
 *  - 提供访问 rrNodes、指定引脚/导线及邻居的接口。
 */
#ifndef FPGATILE_H
#define FPGATILE_H
